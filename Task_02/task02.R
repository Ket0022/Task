setwd('C://Users//kthom//Desktop//Task_02')
...
avgMilk <- mean(cyrus3$value[cMass])
...
totalcMass <- tapply(cyrus3$value[Mass], cyrus3$age[cMass], sum)
numcMass <- tapply(cyrus3$value[cMass], cyrus3$age[cMass], length)
cor(cyrus3$value [Mass], cyrus3$age [Mass])
cor.test(cyrus3$value[cMass], cMass3$age[cMass])
cyrusCor <- cor.test(cyrus3$value[cMass], beren3$age[cMass])
summary(cyrusCor)
...
plot(as.numeric(names(totalMass)), totalMass , type ="b" , pch=16, xlab=" age in days" , ylab="ounces of milk")
boxplot( beren3$value[Feeds]~ beren3$caregiver[Feeds], xlab= "who gave the bottle" , ylab = "amount of milk consumed (oz)")
boxplot(beren3$value[Mass]~beren3$age)
dev.off()
dev.off()

boxplot(beren3$value[Mass]~beren3$age[Mass])
par(las=1, mar=c(5,5,1,1), mgp=c(2, 0.5, 0), tck=-0.01)
...
legend(1000,6, legend=c("Beren", "Cyrus"), col=c("red","yellow"), lty=2:2,  cex=0.8, title = "Children", text.font=4)
points(cyrus3$age[cMass], cyrus3$value[cMass]/1000, pch=16, col='yellow', type='b')
